import re,requests,urllib3,argparse
...
def jsfind(url):
    domain_list=[]
    domain_re=re.compile(r"^(?=^.{3,255}$)(http(s)?:\/\/)?(www\.)?[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+(:\d+)*(\/\w+\.\w+)*$",re.VERBOSE)
    domain=url.split('/')[2]
    # print(domain);exit(1);
    table1=Table(show_header=True,header_style='bold magenta')
    table2=Table(show_header=True,header_style='bold magenta')
    table1.add_column('domain')
    table2.add_column('JS')
    console.print(Panel('[green] js文件爬取 it will take some time ,please wait ![/]'))
    header={
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.99 Safari/537.36'
    }
    rep=requests.get(url,timeout=3,verify=False,headers=header)
    pattern = re.compile(pattern_raw, re.VERBOSE)
    result = re.finditer(pattern, str(rep.text))
    for i in result:
        jsurl=i.group('url').strip('"').strip("'")
        if jsurl.startswith("http"):
            resul=domain_re.search(jsurl)
            if resul==None:
                continue
            else:
                domain_list.append(resul.group().strip())
            table2.add_row(f'[bold cyan]{jsurl}[bold cyan]')
        else:
            table2.add_row(f'[bold cyan]{jsurl}[bold cyan]')

    domain_list=list(set(domain_list))
    for i in domain_list:
        table1.add_row(f'[bold cyan]{i}[bold cyan]')
...
def dirscan(dir_url):
    ...
    table_dir=Table(show_header=True,header_style='bold magenta')
    not_found=requests.get(dir_url+'/'+'sdsfiuhwehhui.php',headers=header,verify=False)
    with open(files,mode='r',encoding='UTF-8') as f:
        for x in track(f.readlines(),description='dirscan..'):
            x=x.strip()
            urls=dir_url+x
            resp=requests.get(headers=header,timeout=4,url=urls,verify=False)
            if resp.status_code != 404 and len(resp.content) != len(not_found.content):
                dir=f"[green]{urls}[green]"
                table.add_row(dir)
                #print(urls)
    console.print(table)
    
def portscan(url_port,port):
    domain_port_url=''
    repc=re.compile(r'"is_open":(?P<is_open>.*?)}}')
    if url_port.startswith("http"):
        domain_port_url=url_port.split('/')[2]
    else:
        domain_port_url=url_port
    header={
        'Referer':'https://tool.box3.cn/port.html',
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.99 Safari/537.36'
    }
    from_url="https://tool.box3.cn/server/port.php"
    from_resp=requests.post(url=from_url,headers=header,verify=False)

    if int(from_resp.status_code) != 200:
        console.print("[red]无法访问进行端口扫描的网站，可能IP被ban，或者该网站不在运营了 !![red]")
        exit(1)   

    table=Table(show_header=True,header_style='bold magenta')
    table.add_column('port')
    for p in track(port,description='portscan..'):
        datas={
            'data':f'{domain_port_url}',
            'port':int(p),
            'protocol':'tcp',
            'method':'search'
            }
        resps=requests.post(url=from_url,headers=header,verify=False,data=datas).text
        if repc.search(resps).group('is_open')=='1':
            table.add_row(f'[red]{p}[red]')
    console.print(table)

def get_domain(url,domian_):
    do='.'.join(url.split('/')[2].split('.')[1:])
    r=re.compile(r'{"domain":"(?P<do>.*?)","status":(?P<status>.*?),"ip":(?P<ip>.*?)}',re.S)
    for list_ in domain_:
        list_=list_.strip()
        from_url=f'https://phpinfo.me/domain?domain={do}&q={list_}'   #{"domain":"pop.baidu.com","status":500,"ip":null}
        try:    
            resp=requests.get(from_url,timeout=3,verify=False,headers=header).text
            fi=r.finditer(resp)
            for i in fi:
                ip=i.group('ip')
                domain=i.group('do')
                status=int(i.group('status').strip('"').strip("'")) 
                if 200 == status:
...
| '_ \| | | | __| __/ _ \ '__| |_| | | | |
| |_) | |_| | |_| ||  __/ |  |  _| | |_| |
|_.__/ \__,_|\__|\__\___|_|  |_| |_|\__, |
                                    |___/[blue]""")

    parser=argparse.ArgumentParser()
    parser.add_argument('-ps',dest='port_url',required=False,type=str,help='指定端口扫描url')
    parser.add_argument('-psm',dest='port_my_url',required=False,type=int,help="指定端口扫描起始端口号",default=0)
    parser.add_argument('-dsm',dest='port_my_url_d',required=False,type=int,help="指定端口扫描结尾端口号",default=0)
    parser.add_argument('-js',dest='js_url',required=False,type=str,help='扫描js文件')
    #parser.add_argument('-jsh',dest='jsh_url',required=False,type=str,help='深度扫描js文件')
    #parser.add_argument('-cs',dest='c_url',required=False,type=str,help='指定c段扫描url')
    parser.add_argument('-ds',dest='dir_url',required=False,type=str,help='指定目录扫描url')
    parser.add_argument('-f',dest='file',required=False,type=str,help='指定目录扫描字典')
    parser.add_argument('-do',dest='domain',required=False,help='扫描子域名',type=str)
    argv=parser.parse_args()
    if argv.port_url:
        if argv.port_my_url == 0:
            port=[21,22,69,23,25,80,443,8080,53,68,67,110,139,143,161,3389,389,512,513,514,
873,1080,1352,1433,2181,5432,5900,6379,7001,27017,9200,9300,11211,8009,8983,4848,
7002,1352,3306,1433,1521,6379,5432,2700,5000,389,995,993]
            portscan(argv.port_url.split('/')[2],port)
        else:
            la=argv.port_my_url
            ld=argv.port_my_url_d
            port=[int(i) for i in range(la,ld+1)]
            portscan(argv.port_url.split('/')[2],port)
